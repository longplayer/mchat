        ...
        <div class="lightbox-main" @mousedown.stop="hide">
          <div class="lightbox-image-container" @mousedown.stop="lightboxClick">
            <transition :name="slideTransitionName" mode="out-in">
              <img class="lightbox-image" :key="index" :src="filteredImages[index].src" />
            </transition>
          </div>
        </div>
      </div>
...
<script>
// src: https://github.com/am283721/vue-my-photos
export default {
  name: 'Lightbox',
  components: {},
  props: {
  ...
  data() {
    return {
      ...
      index: -1, // Index indicates which photo to display. Default to 1st photo
  ...
  methods: {
    ...
    lightboxClick(ev) {
      if (this.closeOnBackdropClick) {
        ...
        if (
          ev.clientX < (lightboxWidth - actualImageWidth) / 2 ||
          ev.clientX > (lightboxWidth + actualImageWidth) / 2
        ) {
          this.hide()
        }
        // If user clicked above or below the image
          ...
          ev.clientY < (lightboxHeight - actualImageHeight) / 2 ||
          ev.clientY > (lightboxHeight + actualImageHeight) / 2
        ) {
          this.hide()
        }
    ...
    getCurrentImage() {
      return this.index >= 0 && this.index < this.filteredImages.length
        ? this.filteredImages[this.index]
        : { name: '', alt: '', filter: '', index: '', src: '', thumb: '' }
    },
    keyEventListener(e) {
      if (this.visible) {
        this.controlsVisible = true
    ...
    preloadNextImage() {
      if (this.has_next()) {
        try {
          let _img = new Image()
          _img.src = this.filteredImages[this.index + 1].src
        } catch (e) {}
      }
    },
    restartCaptionTimer() {
